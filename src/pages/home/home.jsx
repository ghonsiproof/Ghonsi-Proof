...
import logo3 from '../../assets/ghonsi-proof-logos/png-logo/3.png';
import logo4 from '../../assets/ghonsi-proof-logos/svg-logo/1.svg';
import logo5 from '../../assets/ghonsi-proof-logos/svg-logo/2.svg';
import './home.css';

const bubbleData = [
  { 
    name: 'Prosper Ayere', 
    bio: 'CEO of Ghonsi Proof & Web3 Educator', 
    img: prosperImg 
  },
  { 
    name: 'Sarah Johnson', 
    bio: 'UI/UX Designer Specializing in Web3 product designs', 
    img: logo1 
  },
  { 
    name: 'Michael Lee', 
    bio: 'Smart Contract Developer • Rust & Solidity', 
    img: logo2 
  },
  { 
    name: 'Alex Chan', 
    bio: 'Front-End Developer • Rust & Solidity', 
    img: logo3 
  },
  { 
    name: 'Samuel Yang', 
...
function Home() {
  ...
  const allBubbles = [...bubbleData, ...bubbleData, ...bubbleData];
      ...
      <main>
        <section className="h-screen relative overflow-hidden mt-[125px] mb-0">
          <div className="w-full h-screen relative border-2 border-[#C19A4A]/30 rounded-xl box-border p-5">
            ...
            <p className="absolute top-5 left-1/2 -translate-x-1/2 z-50 text-[#C19A4A] text-sm font-medium text-center tracking-wide">Click on any bubble to explore verified web3 professionals</p>
            ...
            <div className="bubbles">
              {allBubbles.map((bubble, index) => (
                <div
                  ...
                  onMouseLeave={handleBubbleLeave}
                  style={{
                    border: (selectedBubble === bubble || pinnedBubble === bubble) ? '4px solid #C19A4A' : 'none',
                  }}
                >
        ...
        <section className="bg-[#0B0F1B] py-16 px-5 text-center rounded-lg m-4 border border-white/10 capitalize">
          <div className="text-center mb-8">
            ...
            <p className="text-base text-[#CCC] leading-[1.4] font-bold">Three simple steps to build your verified Web3 reputation</p>
          </div>

          <div className="relative max-w-[400px] mx-auto h-72" id="cardsGallery">
            <div className={`absolute top-0 left-0 w-full h-full bg-[#0B0F1B] rounded-2xl py-8 px-6 shadow-[0_8px_16px_#C19A4A] ${currentSlide === 0 ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition-opacity duration-[800ms] border border-[#C19A4A] flex flex-col justify-center hover:-translate-y-2 hover:shadow-[0_24px_48px_#C19A4A]`} onClick={handleCardClick} onMouseEnter={() => setHoveredCard(0)} onMouseLeave={() => setHoveredCard(null)}>
              <Wallet className={`mb-4 mx-auto transition-colors duration-300 ${hoveredCard === 0 ? 'text-white' : 'text-[#C19A4A]'}`} size={150} />
              <h3 className="text-white text-xl font-bold mb-2.5">Connect</h3>
              <p className="text-sm text-[#CCC] leading-[1.7]">Sign up, connect your wallet and upload your proofs.</p>
              <span className="text-[30px] font-black text-[#C19A4A] mt-9">01</span>
            </div>

            <div className={`absolute top-0 left-0 w-full h-full bg-[#0B0F1B] rounded-2xl py-8 px-6 shadow-[0_8px_16px_#C19A4A] ${currentSlide === 1 ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition-opacity duration-[800ms] border border-[#C19A4A] flex flex-col justify-center hover:-translate-y-2 hover:shadow-[0_24px_48px_#C19A4A]`} onClick={handleCardClick} onMouseEnter={() => setHoveredCard(1)} onMouseLeave={() => setHoveredCard(null)}>
              <Shield className={`mb-4 mx-auto transition-colors duration-300 ${hoveredCard === 1 ? 'text-white' : 'text-[#C19A4A]'}`} size={125} />
              <h3 className="text-white text-xl font-bold mb-2.5">Verify</h3>
              <p className="text-sm text-[#CCC] leading-[1.7]">Sync and verify your work history.</p>
              <span className="text-[30px] font-black text-[#C19A4A] mt-9">02</span>
            </div>

            <div className={`absolute top-0 left-0 w-full h-full bg-[#0B0F1B] rounded-2xl py-8 px-6 shadow-[0_8px_16px_#C19A4A] ${currentSlide === 2 ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition-opacity duration-[800ms] border border-[#C19A4A] flex flex-col justify-center hover:-translate-y-2 hover:shadow-[0_24px_48px_#C19A4A]`} onClick={handleCardClick} onMouseEnter={() => setHoveredCard(2)} onMouseLeave={() => setHoveredCard(null)}>
              <Upload className={`mb-4 mx-auto transition-colors duration-300 ${hoveredCard === 2 ? 'text-white' : 'text-[#C19A4A]'}`} size={150} />
              <h3 className="text-white text-xl font-bold mb-2.5">Share</h3>
              <p className="text-sm text-[#CCC] leading-[1.7]">Instantly share your on-chain portfolio wuth employers and founders.</p>
              <span className="text-[30px] font-black text-[#C19A4A] mt-9">03</span>
            </div>
          </div>
