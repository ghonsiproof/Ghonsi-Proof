...
export const signInWithWallet = async (walletAddress, signatureData) => {
  try {
    ...
    if (fetchError && fetchError.code === 'PGRST116') {
      // User doesn't exist - create minimal user object
      // Note: Database insert is skipped due to RLS policy on users table
      // This will work once you enable Web3 auth in Supabase and configure RLS properly
      isNewUser = true;
      user = {
        wallet_address: walletAddress,
        created_at: new Date().toISOString(),
      };
      console.log('[v0] Created new wallet user (RLS prevented DB insert)', walletAddress);
    } else if (fetchError && fetchError.code !== 'PGRST116') {
      // Only throw if it's a different error (not "no rows found")
      console.error('[v0] Error fetching user:', fetchError);
      // Continue anyway - user can still authenticate with wallet
    }
